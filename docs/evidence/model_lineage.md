...
| 4 | `stage1/2_*_pre_potato_healthy.pt` | Same as #3 + 20 external Target_Spot photos | Same as #3 | n/a (not re-measured) | 38.8% (n=85, test-only, held-out) | No (superseded) | same as row 2/5 |
| 5 | `stage1/2_*_best.pt` | Same as #4 + 100 external Potato_healthy photos | Same as #3 | n/a (not re-measured) | **37.65% (n=85, test-only, held-out)** | **YES — this is what `orchestrator/pipeline.py` calls via `disease_agent.predict_disease()` right now** | 12.04M params, 1.64 GFLOPs/inference, **240.2ms mean latency (std 20.7ms, n=85), 4.16 img/s throughput** (measured, not inherited) |
